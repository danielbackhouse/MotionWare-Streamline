...
@ Purpose: To determine the sleep points from raw data and sleep diaries for given participants
            and use GUI automation to enter sleep times into MotionWare software and perform 
            subsequent sleep analysis by outputting sleep anlysis into excel sheets. 
...
def findSleepTime(sleepRange, actualSleepTime):
         ...
         #Set too sleep variables
          ...
          for index, row in sleepRange.iterrows():
             ...
             # Find the time the participant went to sleep
             if foundSleepTime == False:
                  ...
                  # Look here to switch threshold values for light, activity and both 
                  if darkMotion >= 5 and foundSleepTime == False:
                      #print('dark motion')
...
"""
def findSleepPoint():
    #Get sleep diary sleep times
      sleepData = sleepDataDateTime()
      toSleepTimes = getToSleepDateTimes()
      finishSleepTimes = getFinishSleepDateTimes()
